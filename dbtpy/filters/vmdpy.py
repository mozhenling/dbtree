...
def  vmd(f, alpha, K, tau=0.0, DC=0, init=1, tol=1e-07, maxIterNum=5, omega_init=None):
    """
    u,u_hat,omega = vmd(f, alpha, tau, K, DC, init, tol, maxIterNum=10, omega_init=None)
    #--------------------------------------------------------------------------
    Variational mode decomposition
    Python implementation by Vinícius Rezende Carvalho - vrcarva@gmail.com
    and modified by by Zhenling Mo email: zhenlmo2-c@my.cityu.edu.hk
  ...
  """
    ...
    while ( uDiff > tol and  n < Niter-1 ): # not converged and below iterations limit
        ...
        # Dual ascent
        lambda_hat[n+1,:] = lambda_hat[n,:] + tau*(f_hat_plus - np.sum(u_hat_plus[n+1,:,:],axis = 1))
        ...
        uDiff = np.spacing(1)
        for i in range(K):
            uDiff = uDiff + np.abs(np.dot((u_hat_plus[n,:,i]-u_hat_plus[n-1,:,i]),
                                   np.conj((u_hat_plus[n,:,i]-u_hat_plus[n-1,:,i])))) / (np.abs((np.dot(u_hat_plus[n-1,:,i],
                                   np.conj(u_hat_plus[n-1,:,i])))) + nan_inf_preventor)
